...
if ( $hasSearch ) {
    ...
    if (! class_exists('HotelAPI')) {
        // try to load the client if it's in theme or plugin folder
        // adjust path if your HotelAPI class is elsewhere
        if ( file_exists( get_template_directory() . '/hawkapi.php' ) ) {
            require_once get_template_directory() . '/hawkapi.php';
        } elseif ( file_exists( WP_PLUGIN_DIR . '/hotel-plugin/hawkapi.php' ) ) {
            require_once WP_PLUGIN_DIR . '/hotel-plugin/hawkapi.php';
        }
    }
        ...
        if ( empty($mult) || ! isset($mult['data']) ) {
            ...
            if (! empty($regions)) {
                ...
                if ($region_id) {
                    ...
                    $endpoint = 'https://api.worldota.net/api/b2b/v3/search/serp/region/';
            ...
            if ( empty($suggestHotels) ) {
                echo '<div class="col-lg-12"><p>No hotel matches found.</p></div>';
            } else {
                            ...
                            <div class="col-lg-4 py-4">
                                <div class="room-item mmt-card">
                                    ...
                                    <div class="ri-text">
                                        ...
                                        <a class="primary-btn" href="<?php echo esc_url( add_query_arg( array( 'hotel_id' => $hotelId, 'lang' => $lang ), site_url('/hotel-detail/') ) ); ?>">View deals</a>
                                    </div>
                                </div>
                            </div>
                            <?php
