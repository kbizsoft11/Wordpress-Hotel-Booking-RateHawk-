...
$session_check_in = isset($_SESSION['booking_check_in']) ? $_SESSION['booking_check_in'] : '';
$session_check_out = isset($_SESSION['booking_check_out']) ? $_SESSION['booking_check_out'] : '';
$session_adults = isset($_SESSION['booking_adults']) ? intval($_SESSION['booking_adults']) : 1;
$session_children = isset($_SESSION['booking_children']) ? intval($_SESSION['booking_children']) : 0;

// Use the session values to override the default values if they exist
$checkInDate = !empty($session_check_in) ? $session_check_in : $checkInDate;
$checkOutDate = !empty($session_check_out) ? $session_check_out : $checkOutDate;
$checkInDateFormatted = !empty($session_check_in) ? $session_check_in : $checkInDateFormatted;
$checkOutDateFormatted = !empty($session_check_out) ? $session_check_out : $checkOutDateFormatted;

// Update search parameters with session values
if (!empty($session_adults)) {
    $searchParameters['mphb_adults'] = $session_adults;
}
if (!empty($session_children)) {
    $searchParameters['mphb_children'] = $session_children;
}
				...
				<p class="mphb-adults-wrapper mphb-capacity-wrapper">
					...
					<select id="<?php echo esc_attr( 'mphb_adults-' . $uniqueSuffix ); ?>" name="mphb_adults">
						...
						<?php foreach ( range( MPHB()->settings()->main()->getMinAdults(), $maxAdultsCount ) as $value ) { ?>
							...
							<option value="<?php echo esc_attr( $value ); ?>" <?php selected( (string) esc_attr( /*$searchParameters['mphb_adults']*/ $session_adults ), (string) $value ); ?>><?php echo esc_html( $value ); ?></option>
				...
				<p class="mphb-children-wrapper mphb-capacity-wrapper">
					...
					<select id="<?php echo esc_attr( 'mphb_children-' . $uniqueSuffix ); ?>" name="mphb_children" <?php echo $selectDataAttributes; ?>>
						...
						<?php foreach ( range( MPHB()->settings()->main()->getMinChildren(), $maxChildrenCount ) as $value ) { ?>
							...
							<option value="<?php echo esc_attr( $value ); ?>" <?php selected( esc_attr( (string) /*$searchParameters['mphb_children']*/ $session_children ), (string) $value ); ?>><?php echo esc_html( $value ); ?></option>
 ...
 *
 * @param string $htmlId
 * @param string $linkText Optional. '#' by default.
 * @return string
 */
function mphb_tmpl_get_anchor_html_for_current_page( $htmlId, $linkText = '#' ) {
	$anchorLink = sprintf( '%s#%s', add_query_arg( array() ), $htmlId );

	$anchorHtml = sprintf(
		'<a class="mphb-link-anchor" id="%s" href="%s">%s</a>',
		esc_attr( $htmlId ),
		esc_url( $anchorLink ),
		esc_html( $linkText )
	);
