<?php
echo "<!-- MPH OVERRIDE TEST: single-room-type-view loaded from child theme -->";
...
class SingleRoomTypeView extends RoomTypeView {
	...
	public static function renderReservationForm() {
		...
		if ( ! MPHB()->settings()->main()->isBookingDisabled() ) {
			...
			if ( mphb_prices_facade()->getActiveRatesByRoomTypeId( MPHB()->getCurrentRoomType()->getId() ) ) {
	...
	public static function renderDefaultOrForDatesPrice() {
		...
		if ( $searchParameters['mphb_check_in_date'] && $searchParameters['mphb_check_out_date'] ) {
			...
			$hasRates = mphb_prices_facade()->isRoomTypeHasActiveRate(
				MPHB()->getCurrentRoomType()->getOriginalId(),
				\DateTime::createFromFormat( 'Y-m-d', $searchParameters['mphb_check_in_date'] ),
				\DateTime::createFromFormat( 'Y-m-d', $searchParameters['mphb_check_out_date'] )
			);
		}
