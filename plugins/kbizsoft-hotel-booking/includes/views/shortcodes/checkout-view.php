...
class CheckoutView {
	...
	public static function renderGuestsChooser( $reservedRoom, $roomIndex, $roomType, $booking ) {
		...
		if ( count( $booking->getReservedRooms() ) == 1 && MPHB()->settings()->main()->isUseOccupancyPresetsOnCheckout() ) {
			list( $presetAdults, $presetChildren ) = mphb_rooms_facade()->getRoomTypeOccupancyPresetsFromSearch( $roomType );
		} else {
			$presetAdults = $presetChildren = '';
		}

		$presetAdults   = apply_filters( 'mphb_sc_checkout_preset_adults', $presetAdults, $roomType, $reservedRoom, $booking );
		$presetChildren = apply_filters( 'mphb_sc_checkout_preset_children', $presetChildren, $roomType, $reservedRoom, $booking );

		$totalCapacity = $roomType->calcTotalCapacity();

		$maxAdults = min( $adultsCapacity, $totalCapacity );
		

		if ( $presetAdults !== '' ) {
			$maxChildren = min( $childrenCapacity, $totalCapacity - $presetAdults );
		} else {
			$maxChildren = min( $childrenCapacity, $totalCapacity );
		}

		$childrenAllowed = $childrenCapacity > 0 && MPHB()->settings()->main()->isChildrenAllowed();
		...
		<?php } else { ?>
			<input type="hidden" id="<?php echo esc_attr( $idPrefix ); ?>-adults" name="<?php echo esc_attr( $namePrefix ); ?>[adults]" value="<?php echo esc_attr( mphb_get_min_adults() ); ?>">
		<?php } ?>
		...
		<?php } else { ?>
			<input type="hidden" id="<?php echo esc_attr( $idPrefix ); ?>-children" name="<?php echo esc_attr( $namePrefix ); ?>[children]" value="<?php echo esc_attr( mphb_get_min_children() ); ?>">
		<?php } ?>
