...
<?php
/**
 * Modern Account Details Template
 *
 * @since 4.2.0
 */
...
?>
<div class="mphb-container mphb-account-details">
<?php
if ( $customer ) {
    ?>
    <form method="post" class="mphb-card mphb-account-details-form" novalidate>
        <input type="hidden" name="mphb_action" value="update_customer" />
        <input type="hidden" name="customer_id" value="<?php echo (int) $customer->getId(); ?>" />
        <input type="hidden" name="redirect_to" value="<?php echo esc_attr( $redirect ); ?>" />
        <?php wp_nonce_field(); ?>

        <h3 class="mphb-card-title"><?php echo esc_html__( 'Account Details', 'motopress-hotel-booking' ); ?></h3>

        <div class="mphb-grid mphb-form-grid">
            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'First Name', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-first-name" name="first_name" type="text" value="<?php echo esc_attr( $customer->getFirstName() ); ?>" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'Last Name', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-last-name" name="last_name" type="text" value="<?php echo esc_attr( $customer->getLastName() ); ?>" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'Username', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-username" name="username" type="text" value="<?php echo esc_attr( $customer->getUserName() ); ?>" readonly="true" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'Email', 'motopress-hotel-booking' ); ?> <abbr title="<?php esc_html_e( 'Required', 'motopress-hotel-booking' ); ?>">*</abbr></span>
                <input id="mphb-email" name="email" type="email" required value="<?php echo esc_attr( $customer->getEmail() ); ?>" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'Phone', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-phone" name="phone" type="text" value="<?php echo esc_attr( $customer->getPhone() ); ?>" />
            </label>

            <label class="mphb-form-field mphb-full">
                <span class="mphb-label"><?php echo esc_html__( 'Address', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-address1" name="address1" type="text" value="<?php echo esc_attr( $customer->getAddress1() ); ?>" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'Country', 'motopress-hotel-booking' ); ?></span>
                <?php
                $countriesList = MPHB()->settings()->main()->getCountriesBundle()->getCountriesList();
                ?>
                <select id="mphb-country" name="country">
                    ...
                    if ( ! empty( $countriesList ) ) {
                        foreach ( $countriesList as $code => $value ) {
                            $selected = ( $code == strtoupper( $customer->getCountry() ) ) ? 'selected="selected"' : '';
            ...
            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'State / County', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-state" name="state" type="text" value="<?php echo esc_attr( $customer->getState() ); ?>" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'City', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-city" name="city" type="text" value="<?php echo esc_attr( $customer->getCity() ); ?>" />
            </label>

            <label class="mphb-form-field">
                <span class="mphb-label"><?php echo esc_html__( 'Postcode', 'motopress-hotel-booking' ); ?></span>
                <input id="mphb-zip" name="zip" type="text" value="<?php echo esc_attr( $customer->getZip() ); ?>" />
            </label>
        </div>

        <hr class="mphb-divider" />

        <div class="mphb-account-change-password">
            <h4><?php echo esc_html__( 'Change Password', 'motopress-hotel-booking' ); ?></h4>
            <div class="mphb-grid mphb-form-grid-2">
                <label class="mphb-form-field">
                    <span class="mphb-label"><?php echo esc_html__( 'Old Password', 'motopress-hotel-booking' ); ?></span>
                    <input id="mphb-old-password" name="old_password" type="password" />
                </label>
                <label class="mphb-form-field">
                    <span class="mphb-label"><?php echo esc_html__( 'New Password', 'motopress-hotel-booking' ); ?></span>
                    <input id="mphb-new-password" name="new_password" type="password" />
                </label>
                <label class="mphb-form-field">
                    <span class="mphb-label"><?php echo esc_html__( 'Confirm New Password', 'motopress-hotel-booking' ); ?></span>
                    <input id="mphb-confirm-new-password" name="confirm_new_password" type="password" />
                </label>
            </div>
        </div>

        <p class="mphb-form-actions">
            <input type="submit" class="mphb-btn mphb-btn-primary" value="<?php echo esc_html__( 'Save Changes', 'motopress-hotel-booking' ); ?>" />
        </p>
    </form>
    <?php
} else {
    echo '<div class="mphb-card mphb-empty"><p>' . esc_html__( 'You are not allowed to access this page.', 'motopress-hotel-booking' ) . '</p></div>';
}
?>
</div>
